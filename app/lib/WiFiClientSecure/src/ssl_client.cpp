...
int start_ssl_client(sslclient_context *ssl_client, const IPAddress& ip, uint32_t port, const char* hostname, int timeout, const char *rootCABuff, bool useRootCABundle, const char *cli_cert, const char *cli_key, const char *pskIdent, const char *psKey, bool insecure, const char **alpn_protos)
{
    ...
    // PATCH-003: close stale socket before creating a new one.
    // Without this, an internal library retry calls start_ssl_client() again
    // and lwip_socket() overwrites sslclient->socket without closing the old
    // fd, leaking it. After ~8 leaked fds the lwIP socket table (16 slots) is
    // exhausted and all subsequent lwip_socket() calls fail with EAGAIN/errno=11.
