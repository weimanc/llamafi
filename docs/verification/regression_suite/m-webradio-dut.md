...
- **⚠ Known firmware state from host audit (2026-06-20/21) — do NOT misattribute to coexistence/network:**
  - **VU meter does not animate during WebRadio (TASK-220b, still open).** Not a missing poll — the VU meter is synthetic by design (ADR-009) and `vu::tick()` runs only in the Spotify app, gated on Spotify's `isPlaying` (false while WebRadio holds the TLS yield). So T_WR_COEX_01's "VU meter animates" human step **will fail**, and it's a design-reconciliation gap, **not** an audio/touch coexistence problem. Record against TASK-220b; do not start network/coexistence diagnosis for it (BP-038/LL-082).
