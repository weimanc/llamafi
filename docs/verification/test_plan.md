...
Landed 2026-07-17 (`fff0208`, orphaned mid-session by the Fable-5 usage limit, gated + committed post-hoc). Design: `docs/architecture/designs/M-WEBRADIO-SETTINGS.md`. **Suite status 2026-07-17 (evening session): all six tests PASS** — the two NOT RUN entries (01/04) were unblocked by purpose-built debug hooks (`set wrInjectResult` + `get settingsSaveCount`, commit `16b8acc`) and the 05 WEAK PASS was re-run clean; see per-test entries. **The WR-4 coordinate re-derivation audit is now DONE (2026-07-17, later session)** — see its entry below.
...
- **Status**: **PASS 2026-07-17** (second session same day, via the new `set wrInjectResult <country>[,<cap>]` hook, commit `16b8acc` — parks a synthetic `WebRadioStationsResult` directly in the dataTask poll slot, `debugInjectGeocode`/T_PRL_08 pattern, and forces `_pendingStations` so tick() polls it). With a live `NL/192` list of 30 stations loaded, injected `ZZ/55`: firmware logged `[W][webradio] stale station result discarded (ZZ/55, want NL/192)` and `get wrCount` still read `count:30` — the mismatched-echo result was never installed. Expected post-discard residue confirmed too: `pending` stays `1` (the discard branch deliberately does NOT clear `_pendingStations` — it keeps polling for the "real" current request; cleared by the next resume()-path fetch, harmless). Note the injection exercises the install-site identity check itself, which is agnostic to whether the mismatched result arrived from a genuinely-timed stale network fetch or the hook — the un-simulated remainder (a real fetch's echo fields being populated correctly at enqueue time) is covered by every normal fetch (`stations loaded count=N country=X` carries the echo).
...
- **Status**: **PASS 2026-07-17** (second session same day, via the new `get settingsSaveCount` hook, commit `16b8acc` — a hard counter incremented only on a fully-completed SPIFFS write in `SettingsStorage::save()`; the TASK-329 `doc.overflowed()` abort and write-failure paths do not count). Three station changes in one session (`set wrPlay 2`, `3`, `1`) left the counter at `0` after every change — zero per-hop saves. Suspend via `switchApp Clock` (deliberately NOT eject, isolating the coalesced save from `persistPlayerMode`'s independent mode-change save per the WR-6 derived-N note) bumped the counter to exactly `1`. Reboot → boot auto-redirected into WebRadio (persisted playerMode) and `get wrIdx` read `1` immediately post-init(), before any fetch resolved — the final index survived flash round-trip. One coalesced save, correct value persisted.
...
- **Status**: **PASS 2026-07-17 (re-run, second session same day — supersedes the earlier WEAK PASS)**. Correction to the re-run note first: `wrLastHttp` carries no request-attempt counter (fields: http/ok/count/jsonErr/tlsInsecure) — the actual attempt counter is `get dataq`'s `wrEnqueues` (accepted `enqueueWebRadioStations()` calls since boot), which is what this re-run used. Conditions fixed both weaknesses of the original run: a fully-populated list (`wrCount:30`, fresh `NL/192` fetch, outside any rate-limit window) and a hard attempt counter. `wrEnqueues` read `1` before the no-edit Settings round-trip (`switchApp Settings` → `switchApp WebRadio`, no country/cap touch) and `1` after — no refetch was even *attempted*, closing the "attempted-but-rate-limited-to-empty" ambiguity that made the first run weak. `wrCount` stayed 30 throughout. Earlier WEAK PASS record (same date, first session): checked via `wrCount` equality (0 before/0 after) during a rate-limited window — kept here as history, superseded by this run.
...
**WR-4 coordinate re-derivation across settings suites — DONE 2026-07-17.** Audit scope: every regression-suite doc citing hardcoded Settings tap coordinates. Findings: only `docs/verification/regression_suite/app-settings-wire-001.md` had stale ones — its Layout reference table and two test bodies (T-WIRE-MAT-01/02, T-WIRE-STOCK-01) still assumed the pre-Teletext/PlaneRadar/WebRadio 5-app, 26px-row Applications list. `m-clock-styles.md` and `m-pr-locations-dut.md` (also named in the original WR-4 finding) were checked and carry **no** hardcoded Applications-list taps — clean, no fix needed there. Fixed: re-derived all 10 rows at `APP_LIST_ROW_H=min(26,212/10)=21`px (`Spotify=38, Clock=59, Crypto=80, Matrix=101, Life=122, Stock=143, Aquarium=164, Teletext=185, PlaneRadar=206, WebRadio=227`), updated the table and the three stale test-body coordinates, confirmed AppIds against `app/src/appRegistry.h` (was a guess before), and fixed an unrelated arithmetic slip in the category-list Cancel-row comment (not a WR-4 coordinate-drift bug — the category list is unaffected, still 6 rows @ 26px). Live-verified on DUT this session: idx1 (Clock, y=59) tapped correctly opened Clock's settings (its Style row toggled digital→flip, confirmed via `get clockStyle`); idx8 (PlaneRadar, y=206) and idx9 (WebRadio, y=227) were exercised repeatedly and correctly throughout the T-HOME-02/05 and T-CPICK-03 legs this session (building on T-CPICK-01's prior live confirmation of those same two). See `app-settings-wire-001.md` for the updated table.

Related but explicitly out of scope for this pass: **TASK-330** (filed 2026-07-17, Developer-owned, not fixed) is the same coordinate-drift class in `app/tools/run_serialdbg_tests.py`'s `_settings_tap_row()` helper — it still uses the uncompressed 26px formula for the Applications list and mis-taps row 2 (T-SET-07/"Aquarium"). Not touched this session (test-tool code fix, Developer's queue, has its own DUT re-run plan for T-SET-03/06/07) — flagging the cross-reference here so the two coordinate-drift fixes aren't mistaken for duplicates of each other.

---

## Suite: M-HOME-LOCATION — device home = prLocs[0] (X035)

Landed 2026-07-17 (`dcc12bf`). Design: `docs/architecture/designs/M-HOME-LOCATION.md`. This suite's DUT session also independently surfaced TASK-329 (settings.json capacity/silent-truncation bug, unrelated to this feature's own code) — see `tasks.md`. **Update 2026-07-17 (later same day): TASK-329 is fixed and DUT-verified** (capacity 6144, save/load round-trip clean, `saved (doc 1561/6144 B)`) — the persistence-corruption hazard that made T-HOME-02/05 inconclusive and T_PRL-adjacent SPIFFS surgery risky no longer blocks re-runs.

**Update 2026-07-17 (real-data DUT session, snapshot-guarded, human-authorized)**: T-HOME-02 and T-HOME-05 re-run and now **PASS** (the prior inconclusive attempts were blocked by TASK-329-window/navigation issues, both resolved); T-HOME-04 **RUN** against synthetic fixtures (all 3 legs PASS). **Suite fully dispositioned — all six T-HOME tests now PASS** (T-HOME-06 remains PASS-partial as recorded, no change). Session followed the mandatory snapshot protocol: `./run/spiffs pull` before any device work, real `settings.json`/`cal.json` snapshotted aside with timestamps, byte-identical restore confirmed (sha256 match) at session end, production firmware reflashed. Key technique that unblocked both tests: `_prComputeDivKm()`/the Confirm screen render (and hence `get prloc`'s `divKm`) fire as soon as `LookupConfirm` is reached — **before** any Save — so both the divergence-hint leg and (combined with a small in-city refinement) the H-2 mirror-refresh leg could be exercised with Cancel-only (zero persistence) except for one deliberate, authorized Save. See per-test entries below for evidence.
...
- **Status**: **PASS 2026-07-17 (real-data DUT session, snapshot-guarded, human-authorized)**. Root cause of the two prior inconclusive attempts: navigation used the empty-slot `SourceFork` layout (`Lookup` at y=38) against a filled slot, which is the wrong button position (filled slots draw a "Current" row and shift the fork to y0=64, `Lookup` at `(137,84)`) — using the correct filled-slot coordinate this time resolved the divergence cleanly. Sequence: `set prloc active 1` (HH) → `get prloc` confirms `home` unchanged immediately after the switch → slot 0 editor → resubmit label "AMS" unchanged → Lookup → country pick `GB` → Postcode → `set geocode 51.5074 -0.1278 …` (stub, small in-city refinement, VE-PRL-2 isolation, no live network) → submit → Confirm screen reached (`divKm`=0, <500, correctly not a city-scale change) → **Save**. Result: `locs[0].lat/lon` → `51.5074/-0.1278`, `home.lat/lon` → the same, **while `active` was still `1`** — the home mirror refreshed unconditionally exactly per H-2. `locs[0].label` stayed `"AMS"` (coords-only refine, confirms D2). `locs[1]` (HH, the active slot) read back byte-identical to baseline — confirms the active mirror was correctly left alone by a slot-0-only edit. This is the one deliberate, authorized mutation of the session; reverted via the end-of-session snapshot restore (byte-identical, confirmed).
...
- **Status**: **PASS 2026-07-17 (run against synthetic fixtures, real settings.json untouched during this test)**. Three legs, each a crafted `settings.json` pushed via `./run/spiffs push` (esptool hard-reset boots it), `get prloc` read, then the next leg's fixture pushed (no real user data used — labels `SL1/SL2/SL3`, distinct from the device's real `AMS/HH/CAM/WFD`): (1) **seed** — `city:"Berlin"`, `prLocs[0]` at the exact compile default (`52.3676/4.9041`, label `"HOME"`) → booted to `prLocs[0] = {52.5, 13.3667}` (kCities Berlin) — seed fired. (2) **preserve** — same `city:"Berlin"`, `prLocs[0]` user-edited to `(10.0, 20.0)` → booted unchanged at `(10.0, 20.0)` — epsilon guard correctly declined to reseed a real edit. (3) **corrupt/truncated** — `city:"Berlin"` set but the `"time"` block's `lat`/`lon` keys omitted entirely (simulating a truncated file) and `prLocs[0]` at compile default → booted to `prLocs[0] = {52.5, 13.3667}`, **not** `(0,0)` — confirms H-7c: the seed source is the `kCities` table lookup by name, never `g_settings.lat/lon`, so a truncated time block cannot seed home to "the Gulf of Guinea." All three legs' `SettingsStorage: loaded` lines were clean (no parse-error/overflow). Real `settings.json` was restored via the snapshot immediately after leg 3, before any T-HOME-02/05 work began; end-of-session restore/diff covers this too.
...
- **Status**: **PASS 2026-07-17 (real-data DUT session, snapshot-guarded, human-authorized; zero persistence — both legs Cancelled, never Saved)**. Lookup leg: from the T-HOME-02 state, re-entered slot 0's editor → Lookup → country `GB` → Postcode → `set geocode 41.9028 12.4964 …` (stub: Rome, ~1430 km from London) → submit → Confirm reached with `divKm:1437` (>500, hint condition true) → tapped **Cancel** (bar position `(227,210)`) → `get prloc` confirms slot 0 stayed at T-HOME-02's refined coords, the Rome edit was discarded, matching the code's own comment ("Cancel keeps prior coords, nothing persisted"). Manual-path disposition leg: same slot 0, SourceFork → **Manual** button (`(137,136)`, the fork's middle stacked slot) → Lat `41.9028` → Lon `12.4964` → `ManualConfirm` reached → `get prloc`'s `divKm` read **1437 — unchanged from the Lookup leg**, i.e. `_prComputeDivKm()` is provably never invoked on the manual branch (confirmed by direct code read: `_repaintPrManualConfirm()` has no divergence-hint code path at all) — the observable doesn't just fail to show a hint, it doesn't even recompute for the manual edit's actual distance. Tapped **Cancel** (`(204,210)`, the manual-confirm bar's 2nd slot) → nothing persisted. **Disposition decision**: the manual-path gap (OQ3/H-4, TASK-322) is accepted as-is for v1 — it is a known, named, deliberately-scoped limitation (the design's own comment: "no 'Retry' concept for manual entry... nothing to retry against"), not a defect. No follow-up task filed; if a future design wants the hint on the manual path too, `_onPrLonSubmit` would need to call `_prComputeDivKm()` before entering `ManualConfirm`, same as the Lookup path's `_tickPrLookup()` does on success.
...
Landed 2026-07-17 (`13bb3fd`). Design: `docs/architecture/designs/M-COUNTRY-PICKER.md` §4. **Suite fully dispositioned 2026-07-17**: T-CPICK-01/02/04/05 PASS (earlier session); T-CPICK-03's remaining prloc Lookup + both Retry legs PASS (later, real-data DUT session, human-authorized) — see its entry for the script-hygiene note on `prloc_editor_smoke.py`. All five tests now PASS or PASS-with-documented-caveat; no open items.
...
- **Status**: **PASS 2026-07-17, both halves** (serial half overnight; **eyeball half PASSED 2026-07-17 daylight** — human confirmed on DUT: picker opens with current selection `NL` visible and visually highlighted, scrollbar thumb + arrows sane at 249 entries, closed without changing the selection. Same eyeball pass cleared the WIRE2 city label on the Weather TIME tile: correct city shown, no clipping, tile chrome intact — see the PM 2026-07-17 note in tasks.md). At WebRadio's Country row: baseline open (current=`NL`) showed `offset:154, highlightIdx:154`; after `set pick US`, reopening showed `offset:235, highlightIdx:235` (`US` sits later alphabetically) — `offset <= highlightIdx < offset+6` held both times. **Navigation finding (not a CPICK defect)**: reaching the WebRadio row required the Applications app-list row for `WebRadio` (`kConfigurableApps` index 9), whose row height is `S_CONTENT_H(212) / CONFIGURABLE_APP_COUNT(10) = 21px`, not the `23px` a comment in the already-committed `app/tools/prloc_editor_smoke.py` assumes (correct when there were 9 configurable apps, stale since WebRadio became the 10th — that script's hardcoded `(100, 223)` for PlaneRadar now lands one row down, on WebRadio, instead). Flagged for Developer/VE to fix that script's coordinate; not blocking here since this suite navigates fresh each time. **Fixed 2026-07-17 (this commit)**: `prloc_editor_smoke.py`, `prloc_manual_smoke.py` and `prloc_ve_smoke.py` (same stale `(100, 223)` tap) now derive list-row Ys from a mirrored-constants block (`row_y()`, `APP_LIST_ROW_H = min(26, 212//10) = 21`, PlaneRadar idx 8 → tap y 206) citing `settingsSection.h` / `appsSection.h _appListRowH()` / `configurable_apps.h`; host `py_compile` clean, tools not yet re-run on DUT.
