...
**Status**: user data recovered in-session from the session-start snapshot (verified byte-identical + clean reload); TASK-329 filed for the code fix (human deferred, 2026-07-17). BP candidate: pre-session persisted-state snapshot + post-session round-trip check with fully-populated config — awaiting human adoption.
...
**Status**: fixed in-session (TASK-321, commit `d40c04f`); `settingsWidgets.h` doc-comment update landed (commit `a7fab6a`) — adopted as **BP-047** (2026-07-16).
