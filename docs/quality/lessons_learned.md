...
**Context**: TASK-394 (a stale `switchApp 10` literal breaking three tools) was a live bug a human hit by eye on the DUT — but it had already been correctly identified and explicitly flagged in TASK-347's own closing write-up, six days earlier, with the words "Flagging for a separate task." No task was ever filed; the note just sat as prose in a closed task, invisible to any `TASK-NNN`-keyed search. TASK-396 was filed to audit `tasks.md`/`tasks-archive.md` for every instance of this pattern. Full audit (this session): 7 distinct notes found matching the phrasing (`flagged for`, `flagging for`, `no task filed`, `not yet filed`, `separate task`, `follow-up task`); `tasks-archive.md` itself was clean (zero hits across 8673 lines). Of the 7: 3 had already been correctly filed later (just never cross-referenced back to the original note), 1 was already a deliberate, explicitly-reasoned drop ("doesn't warrant tracked backlog on its own" — a good call, not a gap), and **2 were genuine gaps** that had sat unfiled since 2026-07-25 (PlaneRadar fetch response-compression lever, TASK-361's own closeout) and 2026-07-28 (WiFi fallback-cascade recovery delay, TASK-364's own closeout) respectively — now filed as TASK-403/TASK-404.
**Observation**: Both real gaps share the same shape: the flag was written several paragraphs deep inside the *closing* write-up of an unrelated, already-shipping task, as free-form prose with no structural marker beyond the phrase itself. Nothing about `tasks.md`'s own format distinguishes "this closing note contains a new tracked-worthy finding" from "this closing note contains incidental color" — both read identically to a human skimming, and neither is discoverable by task-number search, which is the only search this project's own tooling and process actually indexes on.
**Root cause**: No structural convention exists for flagging new work discovered mid-closeout. `AGENTS.md` already says "PM reads `git log` before planning," but nothing says a closeout note is not actually finished closing until its own flagged follow-up has a number or an explicit drop decision.
**Suggested improvement**: Any task closeout that identifies a new, real, out-of-scope issue must either (a) get a `TASK-NNN` filed in the same editing session, or (b) carry an explicit, greppable `**Deferred, not filed:**` marker line instead of free-form "flagging for..." prose — searchable the same way `TODO(TASK-NNN)` already is in code (BP from LL-095's sibling rule, `docs/quality/best_practices.md` line ~415). A future audit, or a future grep, should not have to re-derive by hand which notes are gaps vs. already-actioned vs. deliberately dropped.
**Status**: proposed, awaiting human decision (candidate below).

## Retrospective — 2026-08-05 — TASK-400/401: a fourth+fifth recurrence of the same DRAM-budget failure class, never once written up despite each prior instance citing the last as precedent

### LL-117 — 2026-08-05 — `.dram0.bss` overflow fixes were being re-derived by trial and error every time, because two false starts (that measure to zero effect) look plausible and nothing documented which fix actually works
**Context**: TASK-400 (`systemSection.h`) and TASK-401 (`wifiSection.h`), same day, both new `SettingsApp` sub-sections, both independently overflowed the `cyd2usb_winamp_debug` build's `.dram0.bss` region at link time. TASK-400 fixed itself cleanly (lazy heap-alloc for a `SButton[2]`, matching cited precedent from `TeletextApp::_nosSource()` and `webRadioApp.h`'s `wrPumpConnectUrlBuf()`). TASK-401 hit the same wall harder: after the same lazy-alloc treatment, still 8 bytes over. Two plausible-looking further fixes were tried and **each measured (via `nm`/`size`, not assumed) to change the link-time total by exactly zero**: (1) merging three small class members into one heap-allocated struct behind a single pointer, and (2) relocating that pointer — and, separately, TASK-400's own already-shipped pointer — from a class member to a file-scope `static`. Both looked like real space savings (removing bytes from a struct's `sizeof`) and neither was: a pointer costs its 4 bytes in `.dram0.bss` identically whether it's a member or a file-scope global, and shrinking one struct's `sizeof` without changing what's actually *resident* doesn't reduce the *sum*. Real fix: shrinking a genuinely over-provisioned, pre-existing static array (`WifiSection::_nets[16]` → `_nets[S_MAX_ROWS]`, i.e. 8) that was never displaying more than 8 entries in the first place — proven behavior-preserving by inspection of the render path, not assumed.
**Observation**: This is at minimum the fourth occurrence of this exact failure class on this board (three prior fixes are cited as precedent directly in code comments: `TeletextApp::_nosSource()`, `webRadioApp.h`'s `wrPumpConnectUrlBuf()`, and TASK-400's own `_confirmBtns()` from earlier the same day) — and not one of the prior three was ever written up here, so each new occurrence re-derives "lazy heap-alloc a pointer" from scratch via code-comment archaeology, with no record of the *harder* case (when lazy-alloc alone isn't enough) or the two dead-end fixes that look right but aren't. `app/mem_manifest.yaml` explicitly scopes itself to heap-allocated JIT-arena "overlay" buffers only ("Only heap-allocated buffers are registered here") — the static `.dram0.bss` segment that keeps overflowing has no tracking mechanism at all.
**Root cause**: No best-practice entry or manifest exists for the `.dram0.bss` failure class, only ad-hoc code comments pointing at the previous incident. Each fix is real engineering effort spent solving an already-solved problem, and — this time — real effort was also spent on two theoretically-reasonable fixes that don't actually work, because nothing recorded that pointer relocation is a dead end.
**Suggested improvement**: (a) A BP entry (candidate below) stating the fix priority order for this board's `cyd2usb_winamp_debug` `.dram0.bss` overflow: first look for existing tracked-but-under-displayed capacity to shrink (the `_nets[16]`-style pattern — often bigger and safer than it looks, since a value used as a soak/insertion-sort candidate pool doesn't need to be as large as the pool depth if only the top-N are ever rendered); only then reach for lazy heap-allocation of the *new* feature's own static footprint; and do not bother trying to relocate an already-necessary pointer between class-member and file-scope — measured twice now to have zero effect. (b) `settings-001`'s cross-feature edge (or a new one) should record the board's current headroom figure the way `[[project_webradio_headroom_finding]]` already tracks it for the WebRadio-side arena, since — like that number — it's a moving target that any future Settings-section addition will hit again.
**Status**: adopted → **BP-053** (human, 2026-08-05).

### LL-116 — 2026-08-05 — A subagent that dies mid-task from a session/credit limit is not a failed task; its uncommitted work is salvageable and re-deriving it from scratch is real waste
**Context**: TASK-401's implementing subagent completed the entire feature (migration, storage, new UI sub-steps, debug getter — later confirmed correct against the design doc and the codebase's real interfaces) and was mid-way through iterating on the `.dram0.bss` overflow above when it hit its session credit limit and terminated without ever reporting back. The orchestrating session's first instinct question was whether to treat this as a failed task and relaunch a fresh subagent with the same brief.
**Observation**: `git status`/`git diff` on the working tree showed the complete, high-quality implementation sitting there uncommitted, plus one clearly-mid-debugging artifact (a temporary compile-time `sizeof()` probe someone would recognize instantly as unfinished, not silently shippable). Reviewing and finishing this in place — verify against the design doc line-by-line, fix the one real remaining defect, re-run the full build gate — took a fraction of the time a from-scratch relaunch would have, and preserved implementation choices (e.g. reading a just-connected WiFi password back out of NVS instead of caching a second plaintext copy) that a fresh agent, working from the same brief but without the first agent's in-context reasoning, might not have arrived at identically.
**Root cause**: A subagent's own terminal status (crashed / hit a limit / never reported) says nothing about whether its *work product* is salvageable — those are independent facts, and treating "didn't finish" as "produced nothing usable" discards real, checkable progress by default.
**Suggested improvement**: On any subagent death (credit limit, crash, or otherwise) with file-editing tool access, check `git status`/`git diff` on its target files *before* deciding whether to relaunch. Review the diff for correctness and completeness against the original brief; if it's substantially complete, finish and verify it directly rather than discarding it. Only relaunch from scratch if the diff is empty, clearly broken beyond quick repair, or absent entirely.
**Status**: adopted → **BP-052** (human, 2026-08-05).
...
**Context**: TASK-332's triage step sent the human to `preview_layout.py` to inspect the new 36×36 icons. It showed no icons at all — it still rendered 5×6 TEXT.BMP letter glyphs (S/C/W/$/M/G), its pre-M-TASKBAR-ICONS placeholder, ~5 weeks after real baked icons shipped. Fixing that surfaced the second drift: `preview_common.py` hardcoded `TASKBAR_ICON_W/H = 24` as a "mirrors firmware shell_layout.h" comment-only contract — and `preview_layout.py --export` *writes* `shell_layout.h` from those mirrors, so one habitual export after the ADR-051 bump would have silently regressed the icon budget to 24 (and clobbered the header's ADR annotations).
**Observation**: Third member of a now-established class: the 2026-07-18 M-PR-LOCATIONS audit already flagged the 21px tap-row formula biting three host tools (e929792, TASK-330). The class: host tools encode firmware truth **by copy** (constants, geometry, render behavior) with nothing but a comment binding them; the copies rot invisibly because host tools have no gate — they're only exercised when a human reaches for them, which is exactly when they must not lie. Note the asymmetry: `gen_taskbar_icons.py` *parses* the header and needed zero changes for the size bump; the two tools that *mirrored* it both broke.
**Root cause**: No forcing function ties host-tool mirrors to the generated headers they mirror. `--export` makes it worse than stale display: a drifted mirror becomes a write-path regression.
**Suggested improvement**: (a) Host tools read firmware constants by **parsing the generated header** (the `gen_taskbar_icons.py`/`gen_icon_drafts.py`/`gen_icon_natives.py` pattern), never by mirrored literals — TASK-335 migrates `preview_common.py`. (b) When a generated header's values/semantics change, grep `app/tools/` for the define names in the same pass (cheap; the 118dc07 commit did this by luck, not process). (c) Preview tools that render firmware-owned visuals should render from the same source assets the bake uses, not hand-rolled placeholders that "look close enough".
**Status**: both drifts fixed in-session (mirror bumped 118dc07; real-icon rendering 48e724d); TASK-335 filed for the structural fix (parse, don't mirror) + a feature_inventory description sweep (taskbar-001 still documents "S/C/W/$/M/G via TFT font 4" — two generations stale); host-first approval loop flagged to human as BP candidate

## Retrospective — 2026-07-17 — TASK-329 silent settings truncation: real data loss caught by luck, recovered by an ad-hoc snapshot

### LL-113 — 2026-07-17 — "SettingsStorage: loaded" with no error proved nothing: a valid-but-truncated settings.json loads cleanly into compile defaults, and only a lucky mid-session read caught the data loss
**Context**: TASK-329 (filed this session, fix deferred): `SettingsStorage::load()`/`save()` both use a fixed `DynamicJsonDocument doc(3072)` with no `doc.overflowed()` check. The overnight feature slices (WIRE2 G1-G5, M-HOME-LOCATION, M-WEBRADIO-SETTINGS, M-COUNTRY-PICKER) all grew the persisted schema; on a fully-populated real device the tree exceeded 3072 bytes. ArduinoJson v6 silently no-ops past capacity, so `save()` wrote a **syntactically valid but truncated** `settings.json` — no crash, no error line; on the next load the missing keys fell through per-field `containsKey()` checks to compile defaults. Real user data (all 4 `prLocs` slots, `dispLevel`, `teletextPage`, `webRadioBitrateCap`) was lost.
**Observation**: This entry is about the process gap, not the code bug (that fix is TASK-329's). Detection was pure luck — the operator happened to run `get prloc` mid-session and noticed real location labels gone. "SettingsStorage: loaded" with no error is NOT evidence the persisted state survived: a truncated-but-valid JSON is indistinguishable from a healthy one at the log surface, because the per-field default fallback is *designed* to absorb missing keys silently. Worse, a fresh-default config fits in any capacity by construction — every round-trip check run against default state would have passed green while the real device was eating data. Recovery was possible ONLY because a `settings.json` snapshot happened to be captured at session start (`run/spiffs pull`) — an ad-hoc habit, not a mandated step.
**Root cause**: No step in the DUT workflow verifies that persisted state round-trips with a FULLY-POPULATED real config after schema-growing feature work + heavy reboot cycling, and no session-start persisted-state snapshot is required — the failure class (silent truncation to defaults) is exactly the one that fresh-default checks and "loaded, no error" log reads structurally cannot see.
**Suggested improvement**: (a) After any feature work that adds fields to the persisted schema, explicitly verify the save→load round-trip with a fully-populated real config (all slots filled, long strings in every field) — fresh defaults hide truncation. (b) Take a settings snapshot (`run/spiffs pull`) at session start before any DUT work that reboots repeatedly; it was the only recovery path here. (c) Treat "SettingsStorage: loaded" as absence-of-syntax-error only, never as state-integrity evidence — gate on the actual values (`get prloc` etc.) when integrity matters.
**Status**: user data recovered in-session from the session-start snapshot (verified byte-identical + clean reload); TASK-329 fixed + DUT-verified + closed 2026-07-17 (capacity 6144 + loud `overflowed()` guards, save aborts before touching the file). Adopted → **BP-049** (human, 2026-07-18): pre-session persisted-state snapshot + post-session round-trip check with fully-populated config.

## Retrospective — 2026-07-15 — TASK-321 kit consumer surfaced a silent toolchain gap in TASK-328

### LL-112 — 2026-07-15 — TASK-328's SButton/SSpinner widget kit is not brace-init-safe under this firmware's pinned -std=gnu++11
**Context**: TASK-321 (Settings Locations sub-view + slot editor) is the first real consumer of the TASK-328 widget kit (`settingsWidgets.h`). Every button call site was written as `SButton{ rect, "label", SBtnStyle::Primary }` — reads as ordinary aggregate init, and TASK-328's own close-out claimed "compile-proven via appsSection.h include, run/check 6/6 PASS." `run/check` failed on first real use: `SButton` (and `SSpinner`) declare default member initializers (NSDMI) on their fields, which under `-std=gnu++11` (confirmed via `idedata.json` cxx_flags — this project's ESP32 Arduino toolchain, not overridden anywhere in `platformio.ini`) disqualifies the type from being an aggregate. C++14+ relaxed this rule; C++11 did not. Every 3-arg brace-init site had to be rewritten as individual field assignment (`btn.r = ...; btn.label = ...; btn.style = ...;`).
**Observation**: TASK-328's "compile-proven" claim was true but hollow — the header *included* cleanly with zero widget instantiations in it at the time; the failure mode only exists once a consumer actually constructs the type with the exact syntax that reads most naturally from the type's own field-default-annotated definition. A design pattern (fields with inline defaults, "just brace-init me") silently doesn't work on this toolchain, and nothing about the struct definition itself hints at the restriction.
**Root cause**: "Compiles" was verified at the wrong grain (header inclusion) instead of at actual call-site usage; no test/example call site existed in TASK-328 to catch it. Contributing: this firmware's `-std=gnu++11` pin is easy to forget since GCC on desktop defaults to something newer and would accept the same code silently.
**Suggested improvement**: (a) TASK-327 (migrating wifiSection/ledSection/timeSection/calibrationFlow onto this kit) will hit the same wall at every site — the assignment-not-brace-init requirement should be called out explicitly in `settingsWidgets.h`'s own doc comment (not just discovered per-consumer), and TASK-321's fix already added a note at its own call site. (b) Any future shared struct/header in this codebase with NSDMI fields should either avoid relying on brace-init in its own doc examples, or the header should carry a one-line "gnu++11: construct via field assignment, not `Type{a,b,c}`" note near the struct. (c) "Compile-proven" close-out claims should mean *a real call site compiled*, not *the header was included*.
**Status**: fixed in-session (TASK-321, commit `d40c04f`); `settingsWidgets.h` doc-comment update landed (commit `a7fab6a`) — adopted as **BP-050** (2026-07-16; originally mis-numbered "BP-047" colliding with LL-110's rule, renumbered 2026-07-18).
...
**Status**: fixed in-session (TASK-312, init()→resume() single paint path); TeletextApp shares the latent shape (masked) — flagged; adopted → BP-048 (human, 2026-07-12)
...
**Status**: fixed in-session (TASK-310 extracted `_setPreset()`/`_applyRangeSetting()` etc.); adopted → BP-047 (human, 2026-07-12)
