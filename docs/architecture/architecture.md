...
- **(target — closed ADR-029)** TLS root CA strategy for non-Spotify HTTPS endpoints (`api.open-meteo.com`, `api.coingecko.com`): hardcoded ISRG Root X1 + GTS Root R4 PEMs in `dataTaskCerts.h`, applied via `WiFiClientSecure::setCACert()` + `HTTPClient::begin(client, url)`. `fetchWeather()` and `fetchCrypto()` use `http.getString()` (not `getStream()`) — both endpoints return `Transfer-Encoding: chunked`; `getStream()` on Arduino-ESP32 2.0.17 does not dechunk and ArduinoJson fails to parse (confirmed 2026-05-29, ADR-034 amended). `fetchStockQuote()` / `fetchStockChart()` use `http.getStream()` — Yahoo Finance returns `Content-Length` so streaming is safe (ADR-034).
